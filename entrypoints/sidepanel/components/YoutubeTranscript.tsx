        ...
        messages: [
          {
            role: "system",
            content:
              "あなたは要約の専門家です。以下のテキストをMarkdownを用いて簡潔に箇条書き中心で要約してください。原稿は自動生成されたものであるため、不正確な単語は柔軟に読み替え、要約は日本語で行ってください。最初のタイトルは不要です。最初に動画全体の概要を簡潔に説明後、各トピックは見出し3（###）で始めてください。\n\n**重要**: 各要点には必ず関連するタイムスタンプリンクを含めてください。形式は [MM:SS](timestamp://秒数) です。\n例: [2:30](timestamp://150) のようにマークダウンリンク形式で記載してください。",
          },
          {
            role: "user",
            content: `以下のYouTube動画「${title}」のトランスクリプトを要約してください。各セクションの冒頭にタイムスタンプリンクを必ず付けてください:\n\n${transcriptWithTimestamps}`,
          },
        ],
      });
